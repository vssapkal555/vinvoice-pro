...
void main() {
  test(
    'current invoice template has 25 columns while preserving legacy positions',
    () {
      expect(InvoiceImportColumns.headers.length, 25);
      expect(InvoiceImportColumns.legacyColumnCount, 24);
    },
  );
  ...
  test('important historical column positions remain fixed', () {
    ...
    expect(InvoiceImportColumns.headers[24], 'Service Entry');
    expect(InvoiceImportColumns.serviceEntry, 24);
  });
}
